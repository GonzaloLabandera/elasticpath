...
public class ShippingOptionInfoRepositoryImpl<E extends ShippingOptionEntity, I extends ShippingOptionIdentifier> implements
	...
	private Single<ShippingOptionEntity> getShippingOptionForShipmentDetails(final String scope, final Map<String, String> shipmentDetailsId,
		...
		return shippingServiceLevelRepository.findByGuid(scope, shipmentDetailsId, shippingOptionId)
				.flatMap(shippingServiceLevel -> getShippingPricingSnapshot(scope, shipmentDetailsId, shippingServiceLevel)
						.map(shippingPricingSnapshot -> buildShippingOptionEntity(shippingServiceLevel, shippingPricingSnapshot, locale)));
	}
	...
	private Single<ShippingPricingSnapshot> getShippingPricingSnapshot(final String scope, final Map<String, String> shipmentDetailsId,
		...
		return cartOrderRepository.getEnrichedShoppingCartForShipments(scope, shipmentDetailsId)
				.flatMap(cart -> pricingSnapshotRepository.getShoppingCartPricingSnapshotSingle(cart)
						.map(shoppingCartPricingSnapshot -> shoppingCartPricingSnapshot.getShippingPricingSnapshot(shippingServiceLevel)));
	}

	private ShippingOptionEntity buildShippingOptionEntity(final ShippingServiceLevel shippingServiceLevel,
														   final ShippingPricingSnapshot shippingPricingSnapshot,
														   final Locale locale) {
		return ShippingOptionEntity.builder()
				.withCarrier(shippingServiceLevel.getCarrier())
				.withCost(getCosts(shippingPricingSnapshot, locale))
				.withDisplayName(shippingServiceLevel.getDisplayName(locale, false))
				.withName(shippingServiceLevel.getCode())
				.withShippingOptionId(shippingServiceLevel.getGuid())
				.build();
	}
