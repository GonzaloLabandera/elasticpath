...
Feature: Import store
  ...
  Scenario: Import Payment Configuration with store association
    ...
    And the stores to import of
      | code  | globalization                    | url                                      | name  | state | type | catalog | display_out_of_stock | email_sender_name               | email_sender_address            | store_admin_email          | credit_card_cvv2_enabled | store_full_credit_cards | locales     | currencies | tax_codes   | credit_card_types     | payment_provider_configurations |
      | MOBEE | CAD;en;UTF-8;BC;CA;Europe/London | https://mobee.elasticpath.com/storefront | Mobee | 200   | B2B  | Mobile  | true                 | CustomerService@elasticpath.com | CustomerService@elasticpath.com | StoreAdmin@elasticpath.com | true                     | false                   | fr;fr_ca;en | CAD;EUR    | 502;501;500 | American Express;Visa | 23456                           |
  ...
  Scenario: Unable to import store if the payment configuration is not ACTIVE
    ...
    And the stores to import of
      | code  | globalization                    | url                                      | name  | state | type | catalog | display_out_of_stock | email_sender_name               | email_sender_address            | store_admin_email          | credit_card_cvv2_enabled | store_full_credit_cards | locales     | currencies | tax_codes   | credit_card_types     | payment_provider_configurations |
      | MOBEE | CAD;en;UTF-8;BC;CA;Europe/London | https://mobee.elasticpath.com/storefront | Mobee | 200   | B2B  | Mobile  | true                 | CustomerService@elasticpath.com | CustomerService@elasticpath.com | StoreAdmin@elasticpath.com | true                     | false                   | fr;fr_ca;en | CAD;EUR    | 502;501;500 | American Express;Visa | 23457                           |
    When importing stores with the importexport tool
