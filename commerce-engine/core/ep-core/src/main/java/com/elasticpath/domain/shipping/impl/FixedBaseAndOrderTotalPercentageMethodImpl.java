...
public class FixedBaseAndOrderTotalPercentageMethodImpl extends AbstractShippingCostCalculationMethodImpl {
	...
	public Money calculateShippingCost(final Collection<? extends ShoppingItem> shoppingItems,
		...
		if (!shoppingItems.isEmpty() && hasShippableItem(shoppingItems, productSkuLookup)) {
			...
			shippingCost = Money.valueOf(fixedBase.
					add(percentage.
	 ...
	 */
	private boolean hasShippableItem(
			final Collection<? extends ShoppingItem> shoppingItems, final ProductSkuLookup productSkuLookup) {
