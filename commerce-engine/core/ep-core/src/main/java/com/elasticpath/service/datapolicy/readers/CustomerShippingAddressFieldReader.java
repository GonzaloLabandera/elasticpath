...
/**
 * A data point value reader for reading any customer shipping address field specified by
 * {@link DataPoint}'s dataKey property.
 */
public class CustomerShippingAddressFieldReader extends AbstractDataPointValueReader {
	...
	public String getReadQuery(final Collection<String> dataPointKeys) {
		...
		String query =  "SELECT address.uidPk, address.creationDate, address.lastModifiedDate,"
			...
			.concat(" FROM #TABLE#")
			.concat(" INNER JOIN #alias#.preferredShippingAddressInternal address")
	...
	public Map<String, String> getSupportedFields() {
		return DataPointLocationEnum.CUSTOMER_SHIPPING_ADDRESS.getSupportedFields();
	}
