...
public class CartCategoryAmountDiscountImpl extends AbstractDiscountImpl {
	...
	protected boolean cartItemIsEligibleForPromotion(
		...
		final ProductSku cartSku = getCartItemSku(cartItem);
		return catalogProductInCategory(cartSku.getProduct(), true, compoundCategoryGuid, promotionRuleExceptions)
			&& !promotionRuleExceptions.isSkuExcluded(cartSku);
	}
