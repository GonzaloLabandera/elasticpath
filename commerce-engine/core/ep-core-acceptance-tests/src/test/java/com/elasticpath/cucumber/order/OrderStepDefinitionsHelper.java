...
public class OrderStepDefinitionsHelper {
	...
	public void changeShippingMethodOfOrderPhysicalShipment(final String deliveryOption) {
		...
		List<ShippingServiceLevel> shippingServiceLevels = shippingServiceLevelService.retrieveShippingServiceLevel(
						storeHolder.get().getCode(), phShipment.getShipmentAddress());
		
		for (ShippingServiceLevel shippingServiceLevel : shippingServiceLevels) {
			if (shippingServiceLevel.getDisplayName(order.getLocale(),  true).contains(deliveryOption)) {
				phShipment.setShippingServiceLevelGuid(shippingServiceLevel.getGuid());
				phShipment.setServiceLevel(shippingServiceLevel.getDisplayName(order.getLocale(), true));
			}
		}
	...
	public void moveItemsToNewShipmentFromOrderPhysicalShipment(final List<ShoppingItemDto> itemDtos) {
		...
		newPhysicalShipment.setShippingServiceLevelGuid(phShipment.getShippingServiceLevelGuid());
