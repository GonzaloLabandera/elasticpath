@tax
Feature: taxesForOrderPhysicalShipmentChangeShippingMethod
...
Given a store with an [exclusive] tax jurisdiction of [CA] 
	...
	And with shipping service levels of
		...
		| Canada | Next Business Day | 16.00 |
...
Scenario: Tax Calculation for Changing Shipping Method of Order Physical Shipment 
...
When tax rates are changed to
	...
	And the order physical shipment is modified by changing its shipping method to [Next Business Day]
...
Then I expect that the tax journal should have purchase entries and reversal entries at the corresponding tax rates
		...
		| purchase	  | Order Change	| SHIPPING	| 16.00		 | PST	   | SHIPPING | 1.60	  | 0.10    | CA			  | BC		  |	
		| purchase	  | Order Change	| SHIPPING	| 16.00		 | GST	   | SHIPPING | 1.60	  | 0.10    | CA			  | CA		  |	