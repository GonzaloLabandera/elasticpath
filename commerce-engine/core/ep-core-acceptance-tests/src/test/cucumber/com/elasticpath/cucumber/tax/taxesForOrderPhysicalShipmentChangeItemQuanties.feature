@tax
Feature: taxesForOrderPhysicalShipmentChangeItemQuanties
...
Scenario: Tax Calculation for Changing Item Quantities of Order Physical Shipment 
...
When tax rates are changed to
	...
	And the order physical shipment is modified by changing these items quantities
		| quantity | skuCode   |
		| 2		   | ETX-105PE |	
...
Then I expect that the tax journal should have purchase entries and reversal entries at the corresponding tax rates
		...
		| purchase	  | Order Change	| ETX-105PE | 1198.00	 | PST	   | GOODS	  | 119.80	  | 0.10 	| CA			  | BC		  |
		| purchase	  | Order Change	| ETX-105PE | 1198.00	 | GST	   | GOODS	  | 119.80	  | 0.10    | CA			  | CA		  |	
