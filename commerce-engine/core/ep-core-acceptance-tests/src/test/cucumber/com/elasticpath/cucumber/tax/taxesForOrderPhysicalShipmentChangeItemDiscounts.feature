@tax
Feature: taxesForOrderPhysicalShipmentChangeItemDiscounts
...
Scenario: Tax Calculation for Changing Item Discounts of Order Physical Shipment 
...
When tax rates are changed to
	...
	And the order physical shipment is modified by changing these items discounts
		| discount | skuCode   |
		| 499.00   | ETX-105PE |	
...
Then I expect that the tax journal should have purchase entries and reversal entries at the corresponding tax rates
		...
		| purchase	  | Order Change	| ETX-105PE | 100.00	 | PST	   | GOODS	  | 10.00	  | 0.10 	| CA			  | BC		  |
		| purchase	  | Order Change	| ETX-105PE | 100.00	 | GST	   | GOODS	  | 10.00	  | 0.10    | CA			  | CA		  |	
