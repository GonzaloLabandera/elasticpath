@tax
Feature: taxesForOrderPhysicalShipmentChangeShippingAddress
...
Scenario: Tax Calculation for Changing Shipping Address of Order Physical Shipment 
...
When tax rates are changed to
	...
	And the order physical shipment is modified by changing its shipping address to
		| subCountry | country |
		| AB		 | CA	   |	
...
Then I expect that the tax journal should have purchase entries and reversal entries at the corresponding tax rates
		...
		| purchase	  | Order Change	| ETX-105PE | 599.00	 | HST	   | GOODS	  | 59.90	  | 0.10 	| CA			  | AB		  |
		| purchase	  | Order Change	| ETX-105PE | 599.00	 | GST	   | GOODS	  | 59.90	  | 0.10    | CA			  | CA		  |	
		| purchase	  | Order Change	| SHIPPING	| 7.00		 | HST	   | SHIPPING | 0.70	  | 0.10    | CA			  | AB		  |	
