@tax
Feature: taxesForOrderReturnModification

As Finance, when an order is returned, and the return is modified, I want item taxes to be calculated with original tax rates, 
so I am assured the correct amount of funds returned to the customer are charged or returned, depending upon the modification, 
even if the tax rates have been changed in the interim.
...
Scenario: Tax Calculation for Order Return Modification
...
Then I expect that the tax journal should have purchase entries and reversal entries at the same tax rates as the purchase
		...
		| purchase	  | Order		    | Order SKU		   | ETX-105PE | 1198.00	| PST	  | GOODS	 | 83.86	 | 0.07    | CA			     | BC		 |
		| purchase	  | Order		    | Order SKU		   | ETX-105PE | 1198.00    | GST	  | GOODS	 | 59.90	 | 0.05    | CA			  	 | CA		 |	
		...
		| purchase	  | Return Change   | Order Return SKU | ETX-105PE | 599.00	    | PST	  | GOODS	 | 41.93	 | 0.07    | CA			     | BC		 |
		| purchase	  | Return Change   | Order Return SKU | ETX-105PE | 599.00	    | GST	  | GOODS	 | 29.95	 | 0.05    | CA			  	 | CA		 |	
		| purchase	  | Return Change	| Order Return	   | SHIPPING  | 10.00 	    | PST	  | SHIPPING | 0.70	     | 0.07    | CA			     | BC		 |	
		| purchase	  | Return Change 	| Order Return     | SHIPPING  | 10.00	    | GST	  | SHIPPING | 0.50	     | 0.05    | CA			     | CA		 |
		| reversal	  | Return Change	| Order Return SKU | ETX-105PE | -1198.00   | PST	  | GOODS	 | -83.86	 | 0.07    | CA		         | BC		 |	
		| reversal	  | Return Change	| Order Return SKU | ETX-105PE | -1198.00   | GST	  | GOODS	 | -59.90	 | 0.05    | CA		         | CA		 |	
		| reversal	  | Return Change	| Order Return	   | SHIPPING  | -10.00	    | PST	  | SHIPPING | -0.70	 | 0.07    | CA		         | BC		 |	
		| reversal	  | Return Change	| Order Return	   | SHIPPING  | -10.00	    | GST	  | SHIPPING | -0.50     | 0.05    | CA			     | CA		 |			
