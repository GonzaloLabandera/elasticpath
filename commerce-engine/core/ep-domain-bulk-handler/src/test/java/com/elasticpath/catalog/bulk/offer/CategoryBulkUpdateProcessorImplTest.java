...
/**
 * Tests {@link CategoryBulkUpdateProcessorImpl}.
 */
@RunWith(MockitoJUnitRunner.class)
@SuppressWarnings("PMD.TooManyMethods")
public class CategoryBulkUpdateProcessorImplTest {
	...
	@InjectMocks
	private CategoryBulkUpdateProcessorImpl categoryBulkUpdateProcessor;
	...
	public void shouldCallCatalogServiceReadAllForListOfAllOfferCodes() {
		...
		categoryBulkUpdateProcessor.updateOffers(Arrays.asList(code1, code2));
	...
	public void shouldCallStoreServiceFindStoreWithCodeWithBundleStoreCode() {
		...
		categoryBulkUpdateProcessor.updateOffers(Collections.singletonList(code));
	...
	@Test
	public void shouldCallProjectionServiceBuildProjections() {
		final String code = Utils.uniqueCode(OFFER);
		final String storeCode = Utils.uniqueCode(STORE);
		final Offer offer = mockOffer(code, storeCode);
		...
		when(store.getCatalog()).thenReturn(catalog);
		when(storeService.findStoreWithCode(storeCode)).thenReturn(store);
		when(productBundle.getCatalogs()).thenReturn(Collections.singleton(catalog));
		when(offer.isDeleted()).thenReturn(false);

		when(converter.convert(productBundle, store, catalog)).thenReturn(offer);
		when(projectionService.buildProjections(productBundle, catalog)).thenReturn(Collections.singletonList(offer));
		when(productBundle.getCatalogs()).thenReturn(Collections.singleton(catalog));
		categoryBulkUpdateProcessor.updateOffers(Collections.singletonList(code));

		verify(projectionService).buildProjections(productBundle, catalog);
	}

	@Test
	public void shouldCallCatalogServiceSaveOrUpdateAll() {
		final String code = Utils.uniqueCode(OFFER);
		final String storeCode = Utils.uniqueCode(STORE);
		final Offer offer = mockOffer(code, storeCode);
		...
		verify(catalogService).saveOrUpdateAll(Collections.singletonList(offer));
	}

	@Test
	public void shouldNotCallSaveOrUpdateAllWithEmptyListWhenCatalogServiceReturnsEmptyOffersList() {
		final String code = Utils.uniqueCode(OFFER);
		when(catalogService.readAll(OFFER_IDENTITY_TYPE, Collections.singletonList(code))).thenReturn(Collections.emptyList());

		categoryBulkUpdateProcessor.updateOffers(Collections.singletonList(code));

		verify(catalogService, never()).saveOrUpdateAll(Collections.emptyList());
	}
