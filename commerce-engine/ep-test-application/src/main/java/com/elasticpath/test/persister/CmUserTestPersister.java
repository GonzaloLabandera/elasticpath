...
public class CmUserTestPersister {
	...
	public CmUserTestPersister(final BeanFactory beanFactory) {
		...
		cmUserService = beanFactory.getSingletonBean(ContextIdNames.CMUSER_SERVICE, CmUserService.class);
		userRoleService = beanFactory.getSingletonBean(ContextIdNames.USER_ROLE_SERVICE, UserRoleService.class);
	}
	...
	public CmUser createCmUser(final String guid, final String userName,
		...
		CmUser cmUser = beanFactory.getPrototypeBean(ContextIdNames.CMUSER, CmUser.class);
		cmUser.initialize();
		cmUser.setGuid(guid);
			...
			for (String currentUserRoleName : userRoleNamesArray) {
				if (currentUserRoleName != null) {
					...
					if (existingRole == null) {
						UserRole userRole = beanFactory.getPrototypeBean(ContextIdNames.USER_ROLE, UserRole.class);
						userRole.initialize();
