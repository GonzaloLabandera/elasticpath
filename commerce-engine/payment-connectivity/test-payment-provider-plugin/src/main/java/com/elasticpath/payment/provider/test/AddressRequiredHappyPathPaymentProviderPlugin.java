 ...
 */
@Component("addressRequiredHappyPathPaymentProviderPlugin")
@Scope("prototype")
@BillingAddressRequired
public class AddressRequiredHappyPathPaymentProviderPlugin extends AbstractPaymentProviderPlugin implements
	 ...
	 * Plugin config key A.
	 */
	private static final String CONFIG_KEY_A = "Config A";
	/**
	 * Plugin config field A.
	 */
	private static final String CONFIG_KEY_B = "Config B";
	...
	public PaymentInstrumentCreationResponse createPaymentInstrument(final PaymentInstrumentCreationRequest request)
		...
		if (request.getFormData().containsKey(PIC_FIELD_A)
				&& request.getFormData().containsKey(PIC_FIELD_B) && verifyAddress(request.getPICRequestContextDTO())) {
		...
		throw new PaymentInstrumentCreationFailedException(Collections.singletonList(new ErrorMessage(StructuredMessageType.NEEDINFO,
				...
				Collections.singletonMap("reason", PIC_FIELD_A + ", " + PIC_FIELD_B + " and billing address are required"))));
	}
	...
	public String getPaymentVendorId() {
		return "Address Required Happy Path";
	}
	...
	public PICInstructions getPaymentInstrumentCreationInstructions(final PICInstructionsRequest request)
		...
		if (request.getFormData().containsKey(PIC_INSTRUCTION_FIELD_A)
				&& request.getFormData().containsKey(PIC_INSTRUCTION_FIELD_B) && verifyAddress(request.getPICRequestContextDTO())) {
			return new PICInstructions(ImmutableMap.of("control-secret", UUID.randomUUID().toString()),
	...
	private boolean verifyAddress(final PICRequestContextDTO requestContext) {
		AddressDTO addressDTO = requestContext.getAddressDTO();
		return addressDTO != null
				&& addressDTO.getCity() != null
				&& addressDTO.getCountry() != null
				&& addressDTO.getFirstName() != null
				&& addressDTO.getLastName() != null
				&& addressDTO.getPhoneNumber() != null
				&& addressDTO.getStreet1() != null
				&& addressDTO.getStreet2() != null
				&& addressDTO.getSubCountry() != null
				&& addressDTO.getZipOrPostalCode() != null;
	}
