...
/**
 * An action that will display a dialog to capture a comment, then mark the order hold as unresolved.
 */
public class MarkOrderHoldUnresolvableContributionAction extends Action {
	 ...
	 */
	public MarkOrderHoldUnresolvableContributionAction(final OrderHold orderHold, final Order order) {
				...
				new EditOrderHoldDialog(PlatformUI.getWorkbench().getActiveWorkbenchWindow().getShell(), orderHold, viewOnly) {
					...
					protected void okPressed() {
						if (AuthorizationService.getInstance().isAuthorizedWithPermission(orderHold.getPermission())) {
							...
							orderHold.setStatus(OrderHoldStatus.UNRESOLVABLE_PENDING);
							orderHoldService.update(orderHold);
							orderHoldService.markHoldUnresolvable(order, orderHold, cmUser.getUserName(), getComment());
						}
						super.okPressed();
					...
					protected String getInitialMessage() {
						return FulfillmentMessages.get().OrderHoldDialog_UnresolvableHoldMessage;
					}

					@Override
					protected String getWindowTitle() {
						return FulfillmentMessages.get().OrderHoldDialog_UnresolvableHoldWindowTitle;
					}
				};
		dialog.open();
	}
