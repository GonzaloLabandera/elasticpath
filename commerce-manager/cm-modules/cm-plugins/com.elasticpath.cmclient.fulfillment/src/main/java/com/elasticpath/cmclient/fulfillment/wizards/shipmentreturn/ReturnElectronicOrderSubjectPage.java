...
public class ReturnElectronicOrderSubjectPage extends AbstractEPWizardPage<OrderReturnItem> {
	...
	private class SubjectSectionPart extends AbstractCmClientFormSectionPart {
		...
		private void recalculate() {
			...
			BigDecimal shipmentDiscount;
			try {
				shipmentDiscount = new BigDecimal(shipmentDiscountText.getText()).setScale(currency.getDefaultFractionDigits());
			} catch (NumberFormatException ex) {
				shipmentDiscount = BigDecimal.ZERO.setScale(currency.getDefaultFractionDigits());
			}

			orderReturn.setShipmentDiscount(shipmentDiscount);
		...
		public String getColumnText(final Object element, final int columnIndex) {
			...
			switch (columnIndex) {
				...
				case SKUOPTIONS_COLUMN_INDEX:
					return orderReturnSku.getOrderSku().getDisplaySkuOptions();
