 ...
 */
public class SearchProductViewContentProvider extends ProductListViewContentProvider {
	 ...
	 */
	public SearchProductViewContentProvider(final AbstractProductListView productListView) {
	...
	private boolean isValidEvent(final SearchResultEvent<Product> event) {
		...
		if (source instanceof ProductSearchViewTab || source instanceof SearchProductListView) {
			return true;
		}
		if (source instanceof EpColumnSorterControl) {
			EpColumnSorterControl control = (EpColumnSorterControl) source;
			return (control.getSearchJobRequestorView() instanceof SearchProductListView);
		}
		return false;
	}
}
