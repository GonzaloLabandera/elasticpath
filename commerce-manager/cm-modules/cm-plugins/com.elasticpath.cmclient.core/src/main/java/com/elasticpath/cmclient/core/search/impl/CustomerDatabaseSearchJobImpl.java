...
/**
 * Search for Customers using database queries.
 */
public class CustomerDatabaseSearchJobImpl extends AbstractSearchJobImpl implements SearchJob {

	private static final Logger LOG = LogManager.getLogger(CustomerDatabaseSearchJobImpl.class);
	 ...
	 */
	public CustomerDatabaseSearchJobImpl(final SearchItemsLocator<Customer> locator, final Display display) {
		super(locator);
	...
	protected IStatus run(final IProgressMonitor monitor) {
		...
		try {
			...
			synchronized (getSearchCriteriaQueue()) {
				searchCriteria = (CustomerSearchCriteria) getCriteria();
			}
			if (searchCriteria.getLocale() == null) {
				display.syncExec(() -> searchCriteria.setLocale(CorePlugin.getDefault().getDefaultLocale()));
			}

			long totalItemsCount = customerService.getCustomerCountBySearchCriteria(searchCriteria);
			monitor.worked(COUNT_UNITS_WORK);
			
			if (LOG.isDebugEnabled()) {
				LOG.debug("Searching for items"); //$NON-NLS-1$
			}
			List items = customerService.findCustomersBySearchCriteria(searchCriteria, startIndex, getPagination());
