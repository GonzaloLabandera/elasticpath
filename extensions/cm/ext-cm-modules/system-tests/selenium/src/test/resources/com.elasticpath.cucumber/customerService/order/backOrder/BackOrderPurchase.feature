@regressionTest @customerService @order @backOrder
Feature: Back-order purchase

  Background:
    Given I sign in to CM as admin user
    And I have authenticated as a newly registered shopper

  Scenario Outline: Purchase back-order physical item should reserve full amount
    ...
    And I make a purchase with newly created payment instrument for scope mobee with following skus
      | skuCode                      | quantity |
      | physicalBackOrderProduct_sku | 1        |
  ...
  Scenario Outline: Cannot release shipment of back-order before inventory restocking
    Given I create an unsaved Smart Path Config payment instrument from order supplying following fields:
      | display-name | <PAYMENT_INSTRUMENT_NAME> |
    And I make a purchase with newly created payment instrument for scope mobee with following skus
      | skuCode                      | quantity |
  ...
  Scenario Outline: Back-order is charged when inventory is restocked
    ...
    Examples:
      ...
      | Smart Path Config | unsaved instrument      | physicalBackOrderProduct_sku | $133.13 |
