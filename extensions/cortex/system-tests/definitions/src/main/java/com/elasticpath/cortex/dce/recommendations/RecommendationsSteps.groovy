...
class RecommendationsSteps {

	@When('^I retrieve the recommendation (crosssell|upsell|replacement|warranty|accessory) for this (?:item|offer)$')
	static void gotoRecommendationByType(def recommendationType) {
		Item.navigateToRecommendationType(recommendationType)
	}

	@Then('^I get the ([0-9]*) recommended (?:items|offers)$')
	...
	@When('^I zoom (.+?) into the cross-sells for this (?:item|offer)$')
	...
	@Then('^cross-sell (?:item|offer) is not present$')
