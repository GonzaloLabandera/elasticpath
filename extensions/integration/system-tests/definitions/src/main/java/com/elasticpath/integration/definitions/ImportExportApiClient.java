 ...
 */
public class ImportExportApiClient {
	 ...
	 */
	public ImportExportApiClient(final String baseUrl) {
	 ...
	 *
	 * @param filename the filename containing the XML to import
	 * @param replaced the data to replace containing the XML to import
	 * @throws IOException if there is an issue loading the XML file
	 */
	public void doImportAndReplace(final String filename, final Map<String, String> replaced) throws IOException {
		final Client client = getClient();
		final ClassLoader classLoader = Thread.currentThread().getContextClassLoader();
		final String text = new String(Files.readAllBytes(Paths.get(Objects.requireNonNull(classLoader.getResource(filename)).getPath())));
		final String replacedResult = replaced.entrySet().stream()
				.map(toReplace -> (Function<String, String>) string -> string.replace(toReplace.getKey(), toReplace.getValue()))
		...
		response = client
				...
				.request()
				.accept(MediaType.APPLICATION_XML)
				.post(Entity.entity(replacedResult, MediaType.APPLICATION_XML));
	}
}